#ifndef SOLARSCAPE_NSGAIIIALGORITHM_H
#define SOLARSCAPE_NSGAIIIALGORITHM_H
...
class NSGAIIIAlgorithm final : public GeneticAlgorithm
{
public:
    struct Factories
    {
    ...
    NSGAIIIAlgorithm(std::size_t populationSize, std::size_t generations, const SpecimenComparator& specimenComparator,
                     Factories factories, bool verbose = false);

    ParetoFrontHistory run() const override;

private:
    std::vector<Specimen> createOffspringPopulation(const std::vector<Specimen>& population, const SpecimenComparator& selectionComparator,
                                                    Selection& selection, Crossover& crossover, Mutation& mutation) const;
