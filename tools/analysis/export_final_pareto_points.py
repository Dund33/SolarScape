...
DEFAULT_OUTPUT_PATH = TOOLS_DIR / "out" / "thesis_data" / "final_pareto_points.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export every point from the final Pareto front of each run."
    )
    ...
    parser.add_argument(
        "--feasible-only",
        action="store_true",
        help="Keep only points with fuelConstraintViolation <= --fuel-tolerance.",
    )
...
def main() -> int:
    ...
    output["feasible_only"] = args.feasible_only
    first_columns = [
        "scenario",
        "scenario_label",
        "algorithm",
